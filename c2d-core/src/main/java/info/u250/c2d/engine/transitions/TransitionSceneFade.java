...
final class TransitionSceneFade extends Transition {
    ...
    protected void doTransition(final int halfDurationMillis) {
        outgoing.hide();
        Tween.to(mask, MeshMaskAccessor.Transparency, halfDurationMillis).target(1f).setCallback((type, source) -> {
            doSetMainScene(incoming);
            Tween.to(mask, MeshMaskAccessor.Transparency, halfDurationMillis).target(0).setCallback((t, s) -> {
